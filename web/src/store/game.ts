...
export const useGameStore = defineStore("game", {
  ...
  actions: {
    ...
    handleCellSet(dimension: number, colorPattern: number) {
      if (colorPattern === 0) {
        const redGroup = new Set<number>();
        const blueGroup = new Set<number>();
        const yellowGroup = new Set<number>();
        ...
        for (let i = 1; i < dimension ** 2; i++) {
          if (Math.floor((i - 1) / dimension) + 1 === 1) {
            redGroup.add(i);
          }
          if (Math.floor((i - 1) / dimension) + 1 === 2) {
            blueGroup.add(i);
          }
          if (Math.floor((i - 1) / dimension) + 1 === 3) {
            yellowGroup.add(i);
          }
          if (Math.floor((i - 1) / dimension) + 1 === 4) {
            indigoGroup.add(i);
          }
          if (Math.floor((i - 1) / dimension) + 1 === 5) {
            greenGroup.add(i);
          }
          if (Math.floor((i - 1) / dimension) + 1 === 6) {
            fuchsiaGroup.add(i);
          }
        }
    ...
    getCellClass(cellValue: number, dimension?: number) {
      if (dimension) {
        const cellSet = this.handleCellSet(dimension, this.colorPattern);

        if (cellSet!.redGroup.has(cellValue)) return "bg-red-4 shadow";
        if (cellSet!.blueGroup.has(cellValue)) return "bg-blue-4 shadow";
        if (cellSet!.yellowGroup?.has(cellValue)) return "bg-yellow-4 shadow";
        if (cellSet!.indigoGroup?.has(cellValue)) return "bg-indigo-4 shadow";
        if (cellSet!.greenGroup?.has(cellValue)) return "bg-green-4 shadow";
        if (cellSet!.fuchsiaGroup?.has(cellValue)) return "bg-orange-4 shadow";
