 ...
 */
export const formatDurationInGame = (duration: number) => {
 ...
 * @returns {string} 格式化后的时间字符串(00:00.000)
 */
export const formatDurationInRecord = (duration: number) => {
  const millisecond = duration % 1000;
  const second = Math.floor((duration / 1000) % 60);
  const minute = Math.floor(duration / 1000 / 60) % 60;
 ...
 * @returns {number} 毫秒数
 */
export const formatTimestampByformatedDurationInGame = (
 ...
 * @returns {number} 毫秒数
 */
export const formatTimestampByformatedDurationInRecord = (
  formatedDuration: string
) => {
  const [minute, second, millisecond] = formatedDuration.split(/[:.]/);
 ...
 */
export const formatDataTimeByTimestamp = (timestamp: number) => {
 ...
 */
export const formatDataByTimestamp = (timestamp: number) => {
  const date = new Date(timestamp);
  const year = date.getFullYear();
  const month = date.getMonth() + 1;
  const day = date.getDate();
