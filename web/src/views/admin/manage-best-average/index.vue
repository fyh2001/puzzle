      ...
      return (
        <n-popover z-index={3000} display-directive="if" trigger="hover">
          {{
            ...
            default: () => (
              <div class="w-64 h-64">
                <n-carousel space-between={20} draggable mousewheel show-arrow>
                  {row.recordDetail.map((data: RecordResp) => {
                    return (
      ...
      return (
        <div class="flex flex-col gap-4 mt-4 w-full">
          ...
          >
            ...
            <n-descriptions-item label="用户ID" span={2}>
              {row.userId}
            </n-descriptions-item>
          ...
          <n-descriptions title="记录列表" column={1}>
            <n-descriptions-item>
              <n-collapse>
...
const getData = async () => {
  ...
  const { code, msg } = await adminStore.fetchRecordBestAverageList({
    ...queryForm.value,
    durationRange: queryForm.value.durationRange?.map((item) =>
      item == 0 ? null : (item as number) * 1000
    ),
    rankRange: queryForm.value.rankRange?.map((item) =>
      item == 0 ? null : parseInt(item as string)
    ),
  ...
  <div class="p-4 w-full h-full bg-white rounded-md">
    ...
    <div>
      <n-data-table
        ...
        :data="adminStore.getBestAverageRecordList"
        :bordered="false"
      />
      <n-pagination
        ...
        :item-count="adminStore.getBestAverageRecordTotal"
        :page-slot="3"
