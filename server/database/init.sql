...
CREATE TABLE IF NOT EXISTS `record` (
  ...
  `event` VARCHAR(10) NOT NULL COMMENT 'p15 | p24 | p35 | p48 | p63 | p80',
  ...
  `solution` VARCHAR(255) NOT NULL COMMENT '还原公式',
...
-- 为`record`表的`event`列添加索引，以提高按事件类型进行的查询效率
ALTER TABLE `record` ADD INDEX `idx_event` (`event`);
...
-- 为`record_best_single`表的`event`列添加索引，以提高按事件类型进行的查询效率
ALTER TABLE `record_best_single` ADD INDEX `idx_event` (`event`);

-- 为`record_best_average`表的`event`列添加索引，以提高按事件类型进行的查询效率
ALTER TABLE `record_best_average` ADD INDEX `idx_event` (`event`);

-- 为`record_best_average`表的`type`列添加索引，以提高按类型进行的查询效率
ALTER TABLE `record_best_average` ADD INDEX `idx_type` (`type`);

-- 为`record_best_step`表的`event`列添加索引，以提高按事件类型进行的查询效率
ALTER TABLE `record_best_step` ADD INDEX `idx_event` (`event`);